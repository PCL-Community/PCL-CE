...
public sealed class DependencyCollectorGenerator : IIncrementalGenerator
{
    ...
    {
        var dependencyMap = new Dictionary<string, Dictionary<AttributeTargets, List<MatchResult>>>();
        foreach (var dep in matches)
        {
            var info = dep.Info;
            if (!dependencyMap.TryGetValue(info.Identifier, out var map))
            {
                ...
                dependencyMap[info.Identifier] = map;
            }
