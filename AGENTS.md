...
- **项目名称**：PCL-CE（Minecraft 启动器社区版，基于 .NET 8，注意非 PCL2-CE）
- **核心技术栈**：.NET SDK 10.0 / C# 12.0 / xUnit / VB（启动器本体）
- **项目组成**：
  - `PCL.Core` — 核心库 (C#)
  - `PCL.Core.SourceGenerators` — 源生成器
  - `PCL.Core.Test` — 测试项目 (xUnit)
...
- **推荐 IDE**：JetBrains Rider 2025.3+ / Visual Studio 2026+
...
### 编译时优化
- **正则**：必须用 `GeneratedRegexAttribute`（声明于 `Utils/RegexPatterns.cs`），方法参数传 `Regex` 实例而非 `string`
- **P/Invoke**：必须用 `LibraryImportAttribute` + `partial` + `Span<T>`，推荐 `private`
...
- 核心逻辑写单元测试（xUnit），方法命名 `Should_` 开头，每个方法只测一个功能点
...
1. 禁止直接提交 >200 行或修改 >5 个 `.cs`/`.vb`/`.xaml` 文件的 PR
...
| 测试失败 | 确认 NuGet 包已 restore，数据库已初始化 |
