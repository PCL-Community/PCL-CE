...
{
    public ModBase.SafeList<PCL.CE.Network.DownloadFile> Files;
    ...
    public LoaderDownload(string name, List<PCL.CE.Network.DownloadFile> fileTasks)
    {
        Name = name;
        Files = new ModBase.SafeList<PCL.CE.Network.DownloadFile>(fileTasks ?? new List<PCL.CE.Network.DownloadFile>());
    }
    ...
    {
        if (Input is List<PCL.CE.Network.DownloadFile> inputFiles)
            Files = new ModBase.SafeList<PCL.CE.Network.DownloadFile>(inputFiles);
    ...
    private void Run(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var tasks = Files.Select(async file =>
            {
                ...
                {
                    file.Errors.Add(ex);
                    file.State = PCL.CE.Network.NetState.Interrupted;
    ...
    private async Task ProcessFileAsync(PCL.CE.Network.DownloadFile file, CancellationToken cancellationToken)
    {
        ...
        {
            file.IsCopy = true;
            file.State = PCL.CE.Network.NetState.Finished;
        ...
        file.State = PCL.CE.Network.NetState.Connecting;
    ...
    public void OnFileFinish(PCL.CE.Network.DownloadFile file)
    {
        lock (_fileRemainLock)
        {
            _fileRemain -= 1;
    ...
    public void OnFileFail(PCL.CE.Network.DownloadFile file)
    {
    ...
    public void OnFail(List<Exception> exList)
    {
        ...
        foreach (var file in Files.Where(file => file.State < PCL.CE.Network.NetState.Finished))
        {
            file.State = PCL.CE.Network.NetState.Interrupted;
            file.Speed = 0;
            file.ActiveThreads = 0;
