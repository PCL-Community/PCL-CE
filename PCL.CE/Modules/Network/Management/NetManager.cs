...
public sealed class NetManager
{
    ...
    public ModBase.SafeList<PCL.CE.Network.Loaders.LoaderDownload> Tasks { get; } = new();
    ...
    public void Start(PCL.CE.Network.Loaders.LoaderDownload task)
    {
        lock (LockFiles)
        {
            Tasks.Remove(task);
    ...
    public void Finish(PCL.CE.Network.Loaders.LoaderDownload task)
    {
        lock (LockFiles)
        {
            Tasks.Remove(task);
