...
public static class SimilaritySearch
{
    ...
    {
        if (string.IsNullOrEmpty(source) || string.IsNullOrEmpty(query)) return 0.0;
        ...
        if (sourceSpan.IsEmpty || querySpan.IsEmpty) return 0.0;
        ...
        while (queryIndex < querySpan.Length)
        {
            ...
            for (var sourceIndex = 0; sourceIndex < sourceSpan.Length; sourceIndex++)
            {
                ...
                if (currentMatchLength <= longestMatchLength) continue;
            ...
            {
                for (var i = 0; i < longestMatchLength; i++) usedSourceIndices[bestMatchSourceStartIndex + i] = true;
                ...
                var positionBonus =
                    1.0 + PositionBonusFactor * Math.Max(0, MaxPositionBonusDistance - positionDifference);
        ...
        double minBlurSimilarity = 0.1)
    {
        ...
        if (string.IsNullOrWhiteSpace(query)) return entries; // 或者返回空列表，取决于业务需求

        var queryParts = query.Split(' ', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Select(q => q.ToLower())
            .ToArray();

        if (queryParts.Length == 0) return entries;
