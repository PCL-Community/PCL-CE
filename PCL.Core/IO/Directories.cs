...
public static class Directories
{
    ...
    public static void MoveDirectory(string sourceDir, string targetDir)
    {
        ...
        {
            throw new ArgumentNullException(nameof(sourceDir), "源文件夹路径为空");
        }

        if (string.IsNullOrEmpty(targetDir))
        {
            throw new ArgumentNullException(nameof(targetDir), "目标文件夹路径为空");
        }

        // 规范化路径
        sourceDir = Path.GetFullPath(sourceDir).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar) + Path.DirectorySeparatorChar;
        targetDir = Path.GetFullPath(targetDir).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar) + Path.DirectorySeparatorChar;
        ...
        {
            var fileName = PathUtils.GetFileNameFromPath(filePath);
            File.Move(filePath, Path.Combine(targetDir, fileName));
        }

        foreach (var dirPath in Directory.GetDirectories(sourceDir))
        {
            var dirName = PathUtils.GetFolderNameFromPath(dirPath);
