...
public partial class PageInstanceExport : IRefreshable
{
    ...
    private void ExportConfig(object sender, MouseButtonEventArgs e)
    {
        try
        {
            ...
            HintService.Hint(Lang.Text("Instance.Export.SaveSuccess", configPath), HintType.Finish);
    ...
    private void ReadConfigFile(string configPath)
    {
        try
        {
            ...
            {
                HintService.Hint(Lang.Text("Instance.Export.ConfigInvalid"), HintType.Critical);
                return;
            }
            ...
            // 提示成功
            HintService.Hint(Lang.Text("Instance.Export.ReadSuccess", configPath), HintType.Finish);
        }
            ...
            foreach (var Line in allExtraFiles)
                if (Line.EndsWithF(@"\") || Line.EndsWithF("/"))
                {
                    if (Directory.Exists(Line))
                        ...
                        HintService.Hint(Lang.Text("Instance.Export.ConfigFolderNotFound", Line), HintType.Critical);
                }
                ...
                {
                    HintService.Hint(Lang.Text("Instance.Export.ConfigFileNotFound", Line), HintType.Critical);
                }
