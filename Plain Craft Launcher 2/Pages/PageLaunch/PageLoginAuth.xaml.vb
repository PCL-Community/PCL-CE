...
Public Class PageLoginAuth
    ...
    Private Sub Reload() Handles Me.Loaded
        ...
        If DraggedAuthServer IsNot Nothing Then
            TextServerAddress.Text = DraggedAuthServer
            DraggedAuthServer = Nothing
        End If
    End Sub
    Private Sub BtnBack_Click(sender As Object, e As EventArgs) Handles BtnBack.Click
        RunInUi(Sub()
                    TextServerAddress.Text = Nothing
    ...
    Private Sub BtnLogin_Click(sender As Object, e As EventArgs) Handles BtnLogin.Click
        If Not String.IsNullOrWhiteSpace(TextServerAddress.ValidateResult) Then
            Hint("输入的验证服务器地址无效", HintType.Critical)
            Exit Sub
        End If
        If String.IsNullOrWhiteSpace(TextServerAddress.Text) OrElse String.IsNullOrWhiteSpace(TextName.Text) OrElse String.IsNullOrWhiteSpace(TextPass.Password) Then
            Hint("验证服务器、用户名与密码均不能为空！", HintType.Critical)
            Exit Sub
        End If
        BtnLogin.IsEnabled = False
        BtnBack.IsEnabled = False
        Dim LoginData As New McLoginServer(McLoginType.Auth) With {.BaseUrl = If(TextServerAddress.Text.EndsWithF("/"),
            TextServerAddress.Text & "authserver", TextServerAddress.Text & "/authserver"), .UserName = TextName.Text, .Password = TextPass.Password, .Description = "Authlib-Injector", .Type = McLoginType.Auth}
        RunInNewThread(Sub()
                           Try
    ...
    Private Sub SetServer(sender As Object, e As SelectionChangedEventArgs) Handles ComboServer.SelectionChanged
        Select Case ComboServer.SelectedIndex
            Case 0
                RunInUi(Sub()
                            TextServerAddress.Text = "https://littleskin.cn/api/yggdrasil"
                            If TextServerAddress.IsEnabled Then
                                TextServerAddress.IsEnabled = False
                            End If
                        End Sub)
            Case 1
                RunInUi(Sub()
                            TextServerAddress.Clear()
                            If Not TextServerAddress.IsEnabled Then
                                TextServerAddress.IsEnabled = True
                            End If
    ...
    End Sub
    '获取验证服务器名称
    Private Sub GetServerName() Handles TextServerAddress.LostKeyboardFocus
        Dim serverUriInput = TextServerAddress.Text
        RunInNewThread(Sub()
                           ...
                           End Try
                           RunInUi(Sub()
                                       If serverUri IsNot Nothing Then TextServerAddress.Text = serverUri
