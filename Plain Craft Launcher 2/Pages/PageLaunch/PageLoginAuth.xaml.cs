...
public partial class PageLoginAuth
{
    ...
    private async Task<bool> _TryStartYggdrasilConnectAsync()
    {
        ...
        var metadata = (JsonObject)JsonCompat.ParseNode(await response.AsStringAsync().ConfigureAwait(true));
        ...
        var discoveryMetadata = (JsonObject)JsonCompat.ParseNode(await discoveryResponse.AsStringAsync().ConfigureAwait(true));
        var clientId = _GetClientId(server, discoveryMetadata);
        if (string.IsNullOrWhiteSpace(clientId)) return false;

        LauncherLog.Log($"[Profile] Yggdrasil Connect client id resolved for {new Uri(server).Host}", LauncherLogLevel.Debug);
    ...
    private async Task _CompleteYggdrasilConnectLoginAsync(string server, string discovery, string clientId)
    {
        ...
        {
            LauncherLog.Log(ex, Lang.Text("Launch.Account.Auth.LoginFailed"), LauncherLogLevel.Msgbox,
                userSummary: Lang.Text("Launch.Account.Auth.LoginFailed"));
        }
        finally
        {
            UiThread.Post(_FinishLoginAttempt);
        }
    }
    ...
    internal static async Task<bool> IsOAuthSupportedAsync(string authServerUrl)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccess) return false;
            var metadata = (JsonObject)JsonCompat.ParseNode(await response.AsStringAsync().ConfigureAwait(false));
            var discovery = metadata["meta"]?["feature.openid_configuration_url"]?.ToString()
                            ?? metadata["feature.openid_configuration_url"]?.ToString();
            if (string.IsNullOrWhiteSpace(discovery)) return false;
            ...
            if (!discoveryResponse.IsSuccess) return false;
            var discoveryMetadata = (JsonObject)JsonCompat.ParseNode(await discoveryResponse.AsStringAsync().ConfigureAwait(false));
            var clientId = _GetClientId(server, discoveryMetadata);
        ...
        {
            LauncherLog.Log(ex, "检测 Yggdrasil Connect 支持失败", LauncherLogLevel.Debug);
            return false;
        }
    }
    ...
    private void BtnWebsite_Click(object sender, EventArgs e)
    {
        ...
        LauncherProcess.OpenWebsite(websiteUri.ToString());
    }
