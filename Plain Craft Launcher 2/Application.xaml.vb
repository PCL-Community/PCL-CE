...
#End If
            ...
            Try
                ...
                If Not Directories.CheckPermission(PathTemp) Then Throw New Exception("PCL 没有对 " & PathTemp & " 的访问权限")
            Catch ex As Exception
...
#End If
            ...
            Log($"[Start] 程序路径：{Paths.ExePathWithName}")
            ...
            '检测异常环境
            If Paths.ExePath.Contains(IO.Path.GetTempPath()) OrElse Paths.ExePath.Contains("AppData\Local\Temp\") Then
                MyMsgBox("请将 PCL 从压缩包中解压之后再使用！" & vbCrLf & "在当前环境下运行可能会导致丢失游戏存档或设置，部分功能也可能无法使用！", "环境警告", "我知道了", IsWarn:=True)
            End If
            If Paths.ExePath.ContainsF("wechat_files", True) OrElse Paths.ExePath.ContainsF("WeChat Files", True) OrElse Paths.ExePath.ContainsF("Tencent Files", True) Then
            ...
            '删除旧日志
            For i = 1 To 5
                Dim oldLogFile = $"{Paths.ExePath}PCL\Log-CE{i}.log"
...
#End If
        ...
        Catch ex As Exception
            ...
            Try
                FilePath = Paths.ExePathWithName
            Catch
            End Try
