...
public partial class MyMsgInput
{
    ...
    public MyMsgInput(ModMain.MyMsgBoxConverter Converter)
    {
        ...
        {
            LauncherLogger.Log(ex, "输入弹窗初始化失败", LauncherLogger.LogLevel.Hint);
        }
    ...
    private void Load(object sender, EventArgs e)
    {
        try
        {
            ...
            // 记录日志
            LauncherLogger.Log("[Control] 输入弹窗：" + LabTitle.Text);
        }

        catch (Exception ex)
        {
            LauncherLogger.Log(ex, "输入弹窗加载失败", LauncherLogger.LogLevel.Hint);
        }
    }
