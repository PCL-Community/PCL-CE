...
#Region "自定义窗口"
    ...
    ' Generated by Gemini 3 Pro (20260115)
    Private Function _SizeWndProc(hwnd As IntPtr, msg As Integer, wParam As IntPtr, lParam As IntPtr, ByRef handled As Boolean) As IntPtr
        ...
        If msg = WM_NCHITTEST Then
            ' 1. 提取坐标并转换为 WPF 逻辑坐标
            Dim x As Int16 = CType(lParam.ToInt32() And &HFFFF, Int16)
            Dim y As Int16 = CType((lParam.ToInt32() >> 16) And &HFFFF, Int16)
            Dim windowPoint As Point = Me.PointFromScreen(New Point(x, y))

            ' 2. 定义参数
            Dim offset As Double = 6.0      ' 偏移量：在此距离内不触发缩放
            Dim hitWidth As Double = 5.0     ' 热区宽度：真正触发缩放的带状区域
            
            Dim w As Double = Me.ActualWidth
            Dim h As Double = Me.ActualHeight

            ' 3. 判定鼠标是否处于“总感应区”内（物理边缘 0 到 offset + hitWidth）
            ' 如果鼠标在这个范围内，我们就必须接管控制权，不让系统处理
            Dim isInTotalArea As Boolean = (windowPoint.X >= 0 AndAlso windowPoint.X <= offset + hitWidth) OrElse
                                           (windowPoint.X >= w - (offset + hitWidth) AndAlso windowPoint.X <= w) OrElse
                                           (windowPoint.Y >= 0 AndAlso windowPoint.Y <= offset + hitWidth) OrElse
                                           (windowPoint.Y >= h - (offset + hitWidth) AndAlso windowPoint.Y <= h)

            If isInTotalArea Then
                ' 在感应范围内，我们默认拦截该消息
                handled = True

                ' 4. 精确判定是否落在“向内偏移”后的窄条内
                Dim inLeft As Boolean = (windowPoint.X >= offset AndAlso windowPoint.X <= offset + hitWidth)
                Dim inRight As Boolean = (windowPoint.X <= w - offset AndAlso windowPoint.X >= w - offset - hitWidth)
                Dim inTop As Boolean = (windowPoint.Y >= offset AndAlso windowPoint.Y <= offset + hitWidth)
                Dim inBottom As Boolean = (windowPoint.Y <= h - offset AndAlso windowPoint.Y >= h - offset - hitWidth)
                ...
                ' 6. 关键：如果鼠标在 0-10 之间（物理边缘），强制返回 HTCLIENT，“杀掉”系统默认的边缘缩放
                Return New IntPtr(HTCLIENT)
            End If
            
            ' 如果鼠标在窗口中心深处，不处理 handled，让系统正常处理内部控件点击
        End If
