...
#Region "自定义窗口"
    ...
    Private Shared Function GetWindowRect(hWnd As IntPtr, ByRef lpRect As RECTANGLE) As Boolean
    End Function
    <DllImport("user32.dll")>
    ...
    Private Function _SizeWndProc(hwnd As IntPtr, msg As Integer, wParam As IntPtr, lParam As IntPtr, ByRef handled As Boolean) As IntPtr
        ...
        If msg = WM_NCHITTEST Then
            ...
            ' 3. 计算 DPI 缩放后的偏移量
            ' 如果你想要的是“逻辑 10 单位”，在不同 DPI 下对应的像素不同
            Dim dpi As Single = GetDpiForWindow(hwnd) / 96.0F
            Dim offsetPx As Integer = CInt(6 * dpi)       ' 向内偏移像素
            Dim hitWidthPx As Integer = CInt(5 * dpi)      ' 感应区像素宽度

            ' 4. 在物理像素层面进行判定
            ' 判断鼠标是否在窗口的感应范围内
            Dim isInTotalArea As Boolean = (screenX >= rect.Left AndAlso screenX <= rect.Right) AndAlso
                                           (screenY >= rect.Top AndAlso screenY <= rect.Bottom)

            If isInTotalArea Then
                ' 计算鼠标相对于窗口左上角的物理像素位置
                Dim relX As Integer = screenX - rect.Left
                Dim relY As Integer = screenY - rect.Top
                Dim w As Integer = rect.Right - rect.Left
                Dim h As Integer = rect.Bottom - rect.Top
