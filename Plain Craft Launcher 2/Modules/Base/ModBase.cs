...
public static class ModBase
{
    ...
    public static void ClipboardSet(string text, bool showSuccessHint = true)
    {
        RunInThread(() =>
        {
            ...
            if (success && showSuccessHint) RunInUi(() => ModMain.Hint("已成功复制！", HintType.Finish));
        });
    }
    ...
    public static void Log(string text, LogLevel level = LogLevel.Normal, string title = "出现错误")
    {
        ...
        switch (level)
        {
            ...
            {
                if (modeDebug)
                    ModMain.Hint("[调试模式] " + text, HintType.Info, false);
                break;
            }
            case LogLevel.Hint:
            {
                ModMain.Hint(text, HintType.Critical, false);
                break;
            }
    ...
    public static void Log(Exception ex, string desc, LogLevel level = LogLevel.Debug, string title = "出现错误")
    {
        ...
        switch (level)
        {
            ...
            case LogLevel.Debug:
            {
                var exLine = desc + "：" + ex;
                if (modeDebug)
                    ModMain.Hint("[调试模式] " + exLine, HintType.Info, false);
                break;
            }
            case LogLevel.Hint:
            {
                var exLine = desc + "：" + ex;
                ModMain.Hint(exLine, HintType.Critical, false);
                break;
            }
            case LogLevel.Msgbox:
            {
