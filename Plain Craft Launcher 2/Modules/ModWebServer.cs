...
public static class ModWebServer
{
    ...
    public static bool StartWebServer(string name, HttpServer server)
    {
        ...
        Task.Run(() =>
        {
            LauncherLogger.Log($"[WebServer] 服务端 '{name}' 已启动");
            try
            {
                server.Start();
            ...
            {
                LauncherLogger.Log(ex, $"[WebServer] 服务端 '{name}' 运行出错");
            }
            finally
            {
                try
                ...
                LauncherLogger.Log($"[WebServer] 服务端 '{name}' 已停止");
                lock (_webServers)
                {
    ...
    public class NaidCallbackServer : HttpServer
    {
        ...
        private Task<HttpRouteResponse> HandleStatus(HttpListenerRequest request)
        {
            ...
            try
            {
                ...
                {
                    LauncherLogger.Log($"[OAuth] {_serviceName}: {_status.message}{"\r\n"}{_status.stacktrace}");
        ...
        {
            return HttpRouteResponse.Input(LauncherPaths.GetResourceStream("Images/icon.ico")).AsTask();
        }

        private Task<HttpRouteResponse> HandleComplete(HttpListenerRequest request)
        {
            return HttpRouteResponse.Input(LauncherPaths.GetResourceStream("Resources/oauth-complete.html"), "text/html")
                .AsTask();
        }
    }
        ...
        LauncherDispatcher.RunInNewThread(() =>
            {
                ...
                lock (_webServers)
                {
                    ...
                    LauncherLogger.Log($"[OAuth] {serviceName}: 已开始监听 {server.Port} 端口，正在初始化路由");
                    // 开始响应请求
                    var webServiceName = $"oauth/{serviceName}";
                    if (DisposeWebServer(webServiceName)) LauncherLogger.Log("[OAuth] 已关闭先前认证服务服务端");
                    StartWebServer(webServiceName, server);
                    LauncherLogger.Log($"[OAuth] {serviceName}: 初始化完成，开始响应 HTTP 请求");
                }

                // 打开 OAuth URL
                LauncherShell.OpenWebsite(url.Replace("%r", $"http://localhost:{serverPort}/callback"));
