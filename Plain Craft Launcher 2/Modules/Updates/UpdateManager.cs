...
public static class UpdateManager
{
        ...
        try
        {
            if (IsCurrentVersionBeta && (int)Config.Update.UpdateChannel != 1)
            {
                var isNewerThanStable = RemoteServer.IsLatest(UpdateChannel.stable,
                    LauncherEnvironment.IsArm64System ? UpdateArch.arm64 : UpdateArch.x64, SemVer.Parse(ModBase.VersionBaseName),
                    ModBase.VersionCode);
    ...
    internal static void DownloadLatestPCL(ModLoader.LoaderBase LoaderToSyncProgress = null)
    {
        ...
        if ((LauncherHash.GetFileSHA256(Basics.ExecutablePath) ?? "") == (target.SHA256 ?? "")) // 正在使用的版本符合要求，直接拿来用
        {
            LauncherFileSystem.CopyFile(Basics.ExecutablePath, LatestPCLPath);
            return;
        }
