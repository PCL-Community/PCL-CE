...
public static class ModLaunch
{
        ...
        if (!ModProfile.profileList.Any(x => x.Type == McLoginType.Ms))
        {
            ...
            {                
                                    ...
                                    "https://www.xbox.com/zh-cn/games/store/minecraft-java-bedrock-edition-for-pc/9nxp44l49shj")))
                {
                    case 2:
                    {
                        ModMain.Hint(Lang.Text("Minecraft.Launch.DemoMode"), HintType.Critical);
    ...
    public static bool McLaunchStart(McLaunchOptions options = null)
    {
        ...
        {
            ModMain.Hint(Lang.Text("Minecraft.Launch.Error.AlreadyLaunching"), HintType.Critical);
            isLaunching = false;
            return false;
        }
            ...
            ModInstanceList.McMcInstanceSelected != currentLaunchOptions.instance)
        {
            ...
            {
                ModMain.Hint(Lang.Text("Minecraft.Launch.Error.CannotLaunch", currentLaunchOptions.instance.Desc), HintType.Critical);
                isLaunching = false;
                return false;
            }
    ...
    private static void McLaunchStart(ModLoader.LoaderTask<McLaunchOptions, object> loader)
    {
        ...
        catch (Exception ex)
        {
            if (!ex.Message.StartsWithF("$$"))
                ModMain.Hint(ex.Message, HintType.Critical);
            throw;
        }
        ...
        try
        {
            ...
            switch (launchLoader.State)
            {
                case ModBase.LoadState.Finished:
                {
                    ModMain.Hint(Lang.Text("Minecraft.Launch.Success", ModInstanceList.McMcInstanceSelected.Name), HintType.Finish);
                    break;
                }
                case ModBase.LoadState.Aborted:
                {
                    if (abortHint is null)
                        ...
                        ModMain.Hint(abortHint, HintType.Finish);
        ...
        try
        {
            ...
            if (loginJson["availableProfiles"].AsArray().Count == 0)
            {
                if (data.input.ForceReselectProfile)
                    ModMain.Hint(Lang.Text("Minecraft.Launch.Login.Auth.NoProfileCannotSwitch"), HintType.Critical);
            ...
            if (data.input.ForceReselectProfile && loginJson["availableProfiles"].AsArray().Count == 1)
                ModMain.Hint(Lang.Text("Minecraft.Launch.Login.Auth.OnlyOneProfile"), HintType.Critical);
    ...
    private static void McLaunchJava(ModLoader.LoaderTask<int, int> task)
    {
        ...
        lock (ModJava.javaLock)
        {
            ...
            {
                ModMain.Hint(Lang.Text("Minecraft.Launch.Error.NoJava"), HintType.Critical);
                throw new Exception("$$");
            }
        }
    }
        ...
        if (string.IsNullOrWhiteSpace(worldName) && !string.IsNullOrWhiteSpace(server))
        {
            ...
            else
            {
                ...
                if (ModInstanceList.McMcInstanceSelected.Info.HasOptiFine)
                    ModMain.Hint(Lang.Text("Minecraft.Launch.Error.OptiFineAutoJoinWarning"), HintType.Critical);
            }
        }
