...
public static class ProfileUi
{
    ...
    public static bool HandleMicrosoftCreateProfileError(IdentityModelAuthenticationException exception)
    {
        ...
        UiThread.Invoke(() =>
        {
            if (ModMain.MyMsgBox(Lang.Text("Minecraft.Launch.Login.Microsoft.CreateProfile.Message"),
                    ...
                    Lang.Text("Common.Action.Cancel")) == 1)
                LauncherProcess.OpenWebsite("https://www.minecraft.net/zh-hans/msaprofile/mygames/editprofile");
        });
        return true;
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(name)) return new string('0', 32);
        var iniPath = LauncherPaths.TempWithSlash + @"Cache\Uuid\Mojang.ini";
        var uuid = LauncherIniStore.Shared.Read(iniPath, name);
        ...
        LauncherIniStore.Shared.Write(iniPath, name, uuid);
        return uuid;
    }
    ...
    public static void CreateProfile()
    {
        ...
        {
            string? server = null;
            UiThread.Invoke(() => server = ModMain.MyMsgBoxAuthServer(
        ...
        UiThread.Post(() => ModMain.frmLaunchLeft.RefreshPage(true, type));
    }
    ...
    public static void EditProfileId()
    {
        ...
        if (profile.ProfileType == ProfileType.Microsoft)
        {
            ...
            UiThread.Invoke(() => username = ModMain.MyMsgBoxInput(
            ...
            PCL.Core.App.Basics.RunInNewThread(() =>
            {
                try
                {
                    var check = (JsonObject)JsonCompat.ParseNode(Requester.Fetch(
                    ...
                    var result = (JsonObject)JsonCompat.ParseNode(Requester.Fetch(
                    ...
                    UiThread.Post(() => { ModMain.frmLoginProfileSkin?.Reload(); ModMain.frmLaunchLeft.RefreshPage(true); });
                }
                ...
                {
                    LauncherLog.Log(ex, Lang.Text("Launch.Account.Profile.Error.ChangeId"), LauncherLogLevel.Msgbox,
        ...
        if (profile.ProfileType == ProfileType.Authlib || profile.ProfileType == ProfileType.YggdrasilConnect)
        {
            if (!string.IsNullOrWhiteSpace(profile.Server))
                LauncherProcess.OpenWebsite(profile.Server.Replace("/api/yggdrasil/authserver", "/user/profile"));
            return;
        }

        string? newName = null;
        UiThread.Invoke(() => newName = ModMain.MyMsgBoxInput(Lang.Text("Launch.Account.Profile.EditPlayerId.Title"),
    ...
    public static void EditOfflineUuid(McProfile profile, string? uuid = null)
    {
        ...
        {
            int? type = null;
            UiThread.Invoke(() => type = ModMain.MyMsgBoxSelect([
    ...
    public static string GetOfflineUuid(string userName, bool isSplited = false, bool isLegacy = false)
    {
        if (isLegacy)
        {
            var fullUuid = userName.Length.ToString("X").PadLeft(16, '0') + LauncherStringHash.Compute(userName).ToString("X").PadLeft(16, '0');
    ...
    public static void ChangeSkinMs()
    {
        ...
        PCL.Core.App.Basics.RunInNewThread(() =>
        {
            try
            {
                ...
                while (true)
                {
                    if (ModLaunch.mcLoginMsLoader.State == LoadState.Loading) ModLaunch.mcLoginMsLoader.WaitForExit();
                    if (ModLaunch.mcLoginMsLoader.State != LoadState.Finished) ModLaunch.mcLoginMsLoader.WaitForExit(GetLoginData());
                    ...
                    using var contents = new MultipartFormDataContent
                    {
                        ...
                        { new ByteArrayContent(File.ReadAllBytes(skinInfo.LocalFile)), "file", PathUtils.GetFileNameFromUrlOrPath(skinInfo.LocalFile) }
                    };
                    ...
                    var json = (JsonObject)JsonCompat.ParseNode(result);
            ...
            {
                LauncherLog.Log(ex, Lang.Text("Launch.Account.Profile.Error.ChangeSkin"), LauncherLogLevel.Hint,
