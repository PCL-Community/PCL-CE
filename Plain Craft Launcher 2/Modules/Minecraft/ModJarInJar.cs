...
public static class ModJarInJar
{
    ...
    public static List<ModLocalComp.LocalCompFile> Resolve(string parentPath, ZipArchive jar, int depth = 0)
    {
        ...
        foreach (var nestedPath in nestedPaths.Distinct())
        {
            ...
            {
                LauncherLog.Log(ex, "解析内嵌 Mod 失败（" + parentPath + " -> " + nestedPath + "）", LauncherLogLevel.Developer);
            }
        }
    ...
    private static void _CollectFabricNestedJars(ZipArchive jar, List<string> paths)
    {
        try
        {
            ...
            var obj = (JsonObject)JsonCompat.ParseNode(Files.ReadAllTextOrEmptyAsync(entry.Open()).GetAwaiter().GetResult());
        ...
        {
            LauncherLog.Log(ex, "解析 fabric.mod.json 内嵌清单失败", LauncherLogLevel.Developer);
        }
    }
    ...
    private static void _CollectForgeNestedJars(ZipArchive jar, List<string> paths)
    {
        ...
        {
            LauncherLog.Log(ex, "解析 META-INF/jarjar/metadata.json 内嵌清单失败", LauncherLogLevel.Developer);
        }
    }
}
