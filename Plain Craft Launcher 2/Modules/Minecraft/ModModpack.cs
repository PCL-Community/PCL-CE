...
public static class ModModpack
{
        ...
        double progressIncrement)
    {
        ...
        while (retryCount <= 5)
            try
            {
                ...
                Files.ExtractFileAsync(fileAddress, installTemp,
        ...
        double ProgressIncrement)
    {
        ...
        {
            IniFile.Open(VersionIni).Write("VersionArgumentIndie", 1.ToString()); // 开启版本隔离
            IniFile.Open(VersionIni).Write("VersionArgumentIndieV2", Conversions.ToString(true));
        }

        IniFile.Open(VersionIni).Invalidate(); // 重置缓存，避免被安装过程中写入的 ini 覆盖
    }
            ...
            ModDownloadLoaders.Add(new LoaderTask<int, JArray>("获取 Mod 下载信息", Task =>
            {
                ...
                do
                {
                    ...
                    ret = (JArray)((JObject)ModBase.GetJson(ModDownload.DlModRequest(
                        ...
                        "POST", "{\"fileIds\": [" + string.Join(',', ModList) + "]}", "application/json",
                        allowMirror)))["data"];
        ...
        foreach (var File in (dynamic)Json["files"] ?? Array.Empty<JToken>())
        {
            ...
            if (File["env"] is not null)
                switch (File["env"]["client"].ToString() ?? "")
                {
                    case "optional":
                        {
                            if (ModMain.MyMsgBox("是否要下载可选文件 " + PathUtils.GetFileNameFromPath(File["path"].ToString()) + "？",
            ...
            FileList.Add(new DownloadFile(Urls, TargetPath,
                new FileChecker(actualSize: File["fileSize"].ToObject<long>(),
    ...
    private static LoaderCombo<string> InstallPackCompress(string FileAddress, ZipArchive Archive)
    {
        ...
        var Loader = new LoaderCombo<string>("解压压缩包", new[]
        {
            new LoaderTask<string, int>("解压压缩包", Task =>
            {
                ...
                // 加入文件夹列表
                PageSelectLeft.AddFolder(TargetFolder + ArchiveBaseFolder, PathUtils.GetFolderNameFromPath(TargetFolder),
                    false);
                Thread.Sleep(400); // 避免文件争用
                ...
                try
                {
                    ...
                    foreach (var Patch in Patches)
                    {
                        ...
                        if (PatchJson["compatibleJavaMajors"] is not null)
                        {
                            ...
                            {
                                if (JavaVersion > StringExtension.Val(Java))
                                    continue;
                                // 优先选择主要的版本
                                if (StringExtension.Val(Java) == 21d)
                                {
                                    JavaVersion = 21;
                                    JavaComponent = "java-runtime-delta";
                                }
                                else if (StringExtension.Val(Java) == 17d)
                                {
                                    JavaVersion = 17;
                                    JavaComponent = "java-runtime-gamma";
                                }
                                else if (StringExtension.Val(Java) == 11d)
                                {
                                    JavaVersion = 11;
                                    JavaComponent = null;
                                }
                                else if (StringExtension.Val(Java) == 8d)
                                {
                                    JavaVersion = 8;
        ...
        InstallLoaders.Add(new LoaderTask<string, int>("解压整合包文件", Task =>
        {
            ...
            try
            {
                ...
                if (File.Exists(MMCSetupFile))
                {
                    ...
                    {
                        var PreLaunchCommand = IniFile.Open(MMCSetupFile).Read("PreLaunchCommand");
                    ...
                    if (Conversions.ToBoolean(IniFile.Open(MMCSetupFile).Read("JoinServerOnLaunch",
                            Conversions.ToString(false))))
                    {
                        var ServerAddress = IniFile.Open(MMCSetupFile).Read("JoinServerOnLaunchAddress")
                            .Replace(@"\""", "\"");
                        Config.Instance.ServerToEnter[VersionFolder] = ServerAddress;
                        ModBase.Log("[ModPack] 迁移 MultiMC 实例独立设置：自动进入服务器：" + ServerAddress);
                    }

                    if (Conversions.ToBoolean(IniFile.Open(MMCSetupFile).Read("IgnoreJavaCompatibility",
                            Conversions.ToString(false))))
                    {
                    ...
                    var Logo = Path.GetFileName(IniFile.Open(MMCSetupFile).Read("iconKey"));
                    ...
                    // JVM 参数
                    var JvmArgs = IniFile.Open(MMCSetupFile).Read("JvmArgs");
                    if (!string.IsNullOrEmpty(JvmArgs))
                    {
                        if (Conversions.ToBoolean(IniFile.Open(MMCSetupFile).Read("OverrideJavaArgs",
                                Conversions.ToString(false))))
                        {
