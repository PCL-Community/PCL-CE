...
public static class ModModpack
{
        ...
        bool isOnlineInstall = false)
    {
        ...
        if (ModList.Any())
        {
            ...
            // 获取 Mod 下载信息
            ModDownloadLoaders.Add(new LoaderTask<int, JArray>(
            ...
            // 构造 NetFile
            ModDownloadLoaders.Add(new LoaderTask<JArray, List<DownloadFile>>(
        ...
        bool isOnlineInstall = false)
    {
        ...
        foreach (JProperty Entry in (dynamic)Json["dependencies"] ?? Array.Empty<JToken>())
            switch (Entry.Name.ToLower() ?? "")
            {
                ...
                default:
                {
                    ModMain.Hint(Lang.Text("Minecraft.Download.Modpack.UnknownLoader", Entry.Name, Entry.Value),
                        ModMain.HintType.Critical);
                    break;
                }
            }
